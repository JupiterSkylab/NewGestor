...
def abrir_janela_banco_dados():
    ...
    janela_banco.geometry("336x320")  # Altura aumentada 20% (224 -> 269)
    janela_banco.configure(bg="#F5F7FA")
    janela_banco.resizable(False, False)
    ...
    x = (janela_banco.winfo_screenwidth() // 2) - (336 // 2)  # Ajustado para nova largura
    y = (janela_banco.winfo_screenheight() // 2) - (320 // 2)  # Ajustado para nova altura
    janela_banco.geometry(f"336x320+{x}+{y}")
...
frame_cabecalho.pack(fill="x", pady=(0, 12))  # Reduzido espaçamento
    ...
    separador.pack(fill="x", pady=(0, 15))  # Reduzido espaçamento
    ...
    # Estilo padrão com dimensões ajustadas - CORRIGIDO o pady
    estilo_botao_padrao = {
        ...
        'bd': 0,
        'padx': 15,
        'pady': 8,  # AUMENTADO de 1 para 8 para dar altura adequada aos botões
        'cursor': 'hand2',
    ...
    botao_exportar = tk.Button(frame_botoes, text="📤 Exportar Banco",
                            command=lambda: [exportar_banco(), janela_banco.destroy()],
                            width=14, **estilo_botao_padrao)
    ...
    botao_importar = tk.Button(frame_botoes, text="📥 Importar Banco",
                            command=lambda: [importar_banco(), janela_banco.destroy()],
                            width=14, **estilo_botao_padrao)  # Mesmo width=14 e mesmo estilo
    ...
    frame_rodape = tk.Frame(frame_principal, bg="#F5F7FA")
    frame_rodape.pack(fill="x", side="bottom")

    info_rodape = tk.Label(frame_rodape, text="💡 Pressione ESC para fechar esta janela",
                        font=("Segoe UI", 7), bg="#F5F7FA", fg="#BDC3C7")  # Fonte menor
    info_rodape.pack()
    ...
    janela_banco.bind("<Escape>", lambda e: janela_banco.destroy())

    # Focar no primeiro botão
    botao_exportar.focus_set()

    # Mostra a janela só depois de tudo configurado
...
def visualizar_processo(event=None):
    ...
    try:
        ...
        if data_inicio and isinstance(data_inicio, str) and "/" not in data_inicio:
            try:
        ...
        # ✅ CORREÇÃO: Descrição fica 'normal' para permitir edição
        entrada_descricao.config(state='normal')
...
def ativar_edicao_campo(event):
    ...
    # Apenas foca no campo, sem ativar o botão Atualizar
    widget.config(state='normal')
    widget.focus_set()
    ...
    # Habilita o botão de atualizar
    if tabela.focus() and tabela.item(tabela.focus())['values']:
        botao_cadastrar.config(state='normal',
                               command=lambda: atualizar_processo(tabela.item(tabela.focus())['values'][1]))
...
class ToolTip:
    ...
    def show(self, text):
        ...
        if self.tip_window is None:
            ...
            # Aparência moderna com quebra de linha e melhor contraste
            ...
            # Atualiza posição suavemente (sem ocultar)
            dx = abs(x - self.x)
            dy = abs(y - self.y)
...
def backup_automatico(process_numbers=None):
    try:
        ...
        timestamp = datetime.now().strftime('%tk.Y%m%d_%H%M%tk.S')
        sufixo = ""
        try:
            if process_numbers:
...
try:
    ...
    if 'data_criacao' not in colunas_existentes:
        print("➕ Adicionando coluna 'data_criacao' à tabela promessas...")
        try:
            # Adiciona a coluna sem valor padrão (para evitar o erro)
            cursor.execute('ALTER TABLE promessas ADD COLUMN data_criacao TEXT')
            conn.commit()

            # Preenche a nova coluna com a data e hora atuais para os registros existentes
            cursor.execute(
                "UPDATE promessas SET data_criacao = datetime('now', 'localtime') WHERE data_criacao IS NULL")
            conn.commit()

            print("✅ Coluna 'data_criacao' adicionada e valores preenchidos com sucesso!")
        except Exception as e:
    ...
    if 'data_criacao' not in colunas_existentes:
        print("➕ Adicionando coluna 'data_criacao' à tabela promessas...")
        try:
            # 1️⃣ Adiciona a coluna sem valor padrão (para evitar o erro)
            cursor.execute('ALTER TABLE promessas ADD COLUMN data_criacao TEXT')
            conn.commit()

            # 2️⃣ Preenche a nova coluna com a data e hora atuais para os registros existentes
            cursor.execute(
                "UPDATE promessas SET data_criacao = datetime('now', 'localtime') WHERE data_criacao IS NULL")
            conn.commit()
            ...
            if isinstance(widget, Frame):
                for subwidget in widget.winfo_children():
                    if isinstance(subwidget, Listbox):
                        ...
                        for idx, (rowid, data_exibir, descricao) in enumerate(lembretes):
...
def abrir_lembretes():
    ...
    # Geometria inicial semelhante à foto: largura fixa e altura moderada
    screen_w = janela_lembretes.winfo_screenwidth()
    width = 600
    height = 450
    x = (screen_w - width) // 2
    y = 60
    janela_lembretes.geometry(f"{width}x{height}+{x}+{y}")
    ...
    # Frame principal
    frame_principal = tk.Frame(janela_lembretes, bg="#F5F7FA")
    frame_principal.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    
    # Frame para opções de ordenação
    frame_ordenacao = tk.Frame(frame_principal, bg="#F5F7FA")
    frame_ordenacao.pack(fill=tk.X, pady=(0, 5))
    
    # Variável para controlar a ordenação
    ordenacao_var = tk.StringVar(value="data_criacao_desc")
    
    # Label para ordenação
    tk.Label(frame_ordenacao, text="Ordenar por:", bg="#F5F7FA").pack(side=tk.LEFT, padx=(0, 5))
    
    # Opções de ordenação
    rb_data_criacao_desc = tk.Radiobutton(
        frame_ordenacao, 
        text="Data de cadastro (mais recentes)", 
        variable=ordenacao_var, 
        value="data_criacao_desc",
        command=lambda: carregar_lembretes(ordenacao_var.get()),
        bg="#F5F7FA"
    )
    rb_data_criacao_desc.pack(side=tk.LEFT, padx=5)
    
    rb_data_criacao_asc = tk.Radiobutton(
        frame_ordenacao, 
        text="Data de cadastro (mais antigos)", 
        variable=ordenacao_var, 
        value="data_criacao_asc",
        command=lambda: carregar_lembretes(ordenacao_var.get()),
        bg="#F5F7FA"
    )
    rb_data_criacao_asc.pack(side=tk.LEFT, padx=5)
    
    # Adicionar tooltips para os botões de ordenação
    criar_tooltip(rb_data_criacao_desc, "Exibe os lembretes mais recentes primeiro")
    criar_tooltip(rb_data_criacao_asc, "Exibe os lembretes mais antigos primeiro")
    ...
    # Painel de edição inline (oculto inicialmente)
    modo_edicao_ativo = tk.BooleanVar(value=False)
    id_em_edicao = None

    frame_edicao = tk.Frame(janela_lembretes, bg="#F5F7FA")
    lbl_data_edit = tk.Label(frame_edicao, text="Data prometida (dd/mm/aaaa)", bg="#F5F7FA")
    entry_data_edit = tk.Entry(frame_edicao)
    lbl_desc_edit = tk.Label(frame_edicao, text="Descrição", bg="#F5F7FA")
    text_desc_edit = tk.Text(frame_edicao, height=4, wrap=tk.WORD)

    lbl_data_edit.grid(row=0, column=0, sticky="w", padx=6, pady=(6, 0))
    entry_data_edit.grid(row=0, column=1, sticky="ew", padx=6, pady=(6, 0))
    lbl_desc_edit.grid(row=1, column=0, sticky="nw", padx=6, pady=(6, 0))
    text_desc_edit.grid(row=1, column=1, sticky="ew", padx=6, pady=(6, 6))
    frame_edicao.grid_columnconfigure(1, weight=1)

    def encerrar_edicao(reset_campos=True):
        nonlocal id_em_edicao
        try:
            frame_edicao.pack_forget()
            modo_edicao_ativo.set(False)
            id_em_edicao = None
            if reset_campos:
                entry_data_edit.delete(0, tk.END)
                text_desc_edit.delete("1.0", tk.END)
            # Voltar botão para Editar
            btn_editar.config(text="Editar", command=iniciar_edicao)
        except Exception:
            pass

    def iniciar_edicao():
        """Entra no modo edição e preenche campos com o lembrete selecionado."""
        ...
        idx = selecoes[0]
        nonlocal id_em_edicao
        id_em_edicao = lembrete_ids.get(idx)
        if not id_em_edicao:
            messagebox.showerror("Erro", "Lembrete não encontrado.", parent=janela_lembretes)
            return
        cursor.execute("SELECT data_prometida, descricao FROM promessas WHERE rowid = ?", (id_em_edicao,))
        row = cursor.fetchone()
        data_atual, desc_atual = (row[0] if row else "", row[1] if row else "")
        entry_data_edit.delete(0, tk.END)
        entry_data_edit.insert(0, data_atual or "")
        text_desc_edit.delete("1.0", tk.END)
        text_desc_edit.insert("1.0", desc_atual or "")
        # Mostrar painel e trocar botão para Atualizar
        frame_edicao.pack(fill=tk.X, padx=10, pady=(6, 0))
        btn_editar.config(text="Atualizar", command=atualizar_edicao)
        modo_edicao_ativo.set(True)

    def atualizar_edicao():
        """Atualiza o lembrete em edição e sai do modo edição."""
        if not modo_edicao_ativo.get() or id_em_edicao is None:
            return
        nova_data = entry_data_edit.get().strip()
        nova_desc = text_desc_edit.get("1.0", "end-1c").strip()
        if not nova_data or not nova_desc:
            messagebox.showwarning("Aviso", "Preencha data e descrição.", parent=janela_lembretes)
            return
        try:
            datetime.strptime(nova_data, "%d/%m/%Y")
        except Exception:
            messagebox.showwarning("Aviso", "Data deve estar no formato dd/mm/aaaa.", parent=janela_lembretes)
            return
        try:
            cursor.execute(
                "UPDATE promessas SET data_prometida = ?, descricao = ? WHERE rowid = ?",
                (nova_data, nova_desc, id_em_edicao)
            )
            conn.commit()
            cache.invalidate('count_promessas')
            atualizar_cor_botao_lembrete()
            carregar_lembretes(ordenacao_var.get())
    ...
    # Botão para cancelar edição dentro do painel
    tk.Button(frame_edicao, text="Cancelar edição", command=lambda: encerrar_edicao()).grid(
        row=2, column=1, sticky="e", padx=6, pady=(0, 6)
    )

    # Adicionar tooltip para a lista de lembretes
    criar_tooltip(lista_lembretes, "Clique duplo para visualizar o lembrete completo")
    ...
    # Vincular o menu de contexto ao botão direito na lista
    lista_lembretes.bind("<Button-3>", mostrar_menu_contexto)

    # Frame para botões
    frame_botoes = tk.Frame(janela_lembretes, bg="#F5F7FA")
    frame_botoes.pack(pady=10)

    def carregar_lembretes(ordem="data_criacao_desc"):
        """Carrega os lembretes do banco de dados"""
        global lembrete_ids
        lista_lembretes.delete(0, tk.END)
        
        # Define a ordenação com base no parâmetro
        if ordem == "data_criacao_asc":
            ordem_sql = "ORDER BY datetime(data_criacao) ASC"
        else:  # padrão: data_criacao_desc
            ordem_sql = "ORDER BY datetime(data_criacao) DESC"
        ...
        cursor.execute(f"""
            SELECT 
                ...
                COALESCE(strftime('%d/%m/%Y', data_criacao), data_prometida) AS data_exibir,
                descricao,
                datetime(data_criacao) as data_cadastro
            FROM promessas 
            WHERE pessoa = 'Lembrete'
            {ordem_sql}
        """)
        ...
        for idx, (rowid, data_exibir, descricao, data_cadastro) in enumerate(lembretes):
            # Formatar para exibir a data de cadastro
            data_hora_cadastro = datetime.strptime(data_cadastro, "%Y-%m-%d %H:%M:%S")
            data_cadastro_formatada = data_hora_cadastro.strftime("%d/%m/%Y às %H:%M")
            
            # Adicionar à lista com informação de data de cadastro
            lista_lembretes.insert(tk.END, f"{data_exibir} - {descricao} [Cadastrado em: {data_cadastro_formatada}]")
    ...
    def visualizar_com_debounce():
        ...
        btn_visualizar.config(state="disabled")
        janela_lembretes.after(10, visualizar_lembrete_selecionado)  # Pequeno atraso
        # Reabilita o botão após um pequeno atraso
        janela_lembretes.after(500, lambda: btn_visualizar.config(state="normal"))

    def visualizar_lembrete_selecionado():
        """Visualiza o lembrete selecionado em uma janela modal"""
        ...
        # Busca os detalhes completos do lembrete
        cursor.execute("""
            SELECT data_prometida, descricao, pessoa, data_criacao
            FROM promessas
            WHERE rowid = ?
        """, (id_lembrete,))
        ...
        data_prometida, descricao, pessoa, data_criacao = lembrete

    # Cria janela de visualização (oculta inicialmente para evitar 'piscar')
    janela_visualizar = Toplevel(janela_lembretes)
    janela_visualizar.withdraw()
    janela_visualizar_ativa = janela_visualizar  # Salva referência à janela ativa
    janela_visualizar.title("📋 Detalhes do Lembrete")
    janela_visualizar.geometry("500x450")
    janela_visualizar.resizable(False, False)
    janela_visualizar.configure(bg="#F5F7FA")
    janela_visualizar.transient(janela_lembretes)  # Modal
    janela_visualizar.grab_set()  # Torna a janela modal

    # Centraliza a janela sem exibir primeiro (evita flicker)
    x = (janela_visualizar.winfo_screenwidth() // 2) - (500 // 2)
    y = (janela_visualizar.winfo_screenheight() // 2) - (450 // 2)
    janela_visualizar.geometry(f"500x450+{x}+{y}")
    janela_visualizar.update_idletasks()

        # Frame principal
        frame_principal = tk.Frame(janela_visualizar, bg="#F5F7FA", padx=25, pady=25)
        frame_principal.pack(fill=tk.BOTH, expand=True)

        # Cabeçalho
        tk.Label(frame_principal, text="📋 Detalhes do Lembrete",
              font=("Segoe UI", 14, "bold"), bg="#F5F7FA", fg="#2C3E50").pack(pady=(0, 20))

        # Frame para informações em grid
        frame_info = tk.Frame(frame_principal, bg="#F5F7FA")
        frame_info.pack(fill=tk.X, pady=(0, 20))

        # Data Prometida
        tk.Label(frame_info, text="📅 Data Prometida:",
              font=("Segoe UI", 10, "bold"), bg="#F5F7FA", fg="#34495E",
              width=15, anchor="w").grid(row=0, column=0, sticky=tk.W, pady=8)
        tk.Label(frame_info, text=data_prometida,
              font=("Segoe UI", 10), bg="#F5F7FA", fg="#2C3E50",
              anchor="w").grid(row=0, column=1, sticky=tk.W, pady=8)

        # Data de Criação
        tk.Label(frame_info, text="🕒 Criado em:",
              font=("Segoe UI", 10, "bold"), bg="#F5F7FA", fg="#34495E",
              width=15, anchor="w").grid(row=1, column=0, sticky=tk.W, pady=8)
        tk.Label(frame_info, text=data_criacao,
              font=("Segoe UI", 10), bg="#F5F7FA", fg="#2C3E50",
              anchor="w").grid(row=1, column=1, sticky=tk.W, pady=8)

        # Configurar grid para expandir
        frame_info.grid_columnconfigure(1, weight=1)

        # Descrição
        tk.Label(frame_principal, text="📝 Descrição:",
              font=("Segoe UI", 10, "bold"), bg="#F5F7FA", fg="#34495E",
              anchor="w").pack(anchor="w", pady=(10, 5))

        # Frame para descrição com scroll
        frame_descricao = tk.Frame(frame_principal, bg="white", relief="solid", bd=1)
        frame_descricao.pack(fill=tk.BOTH, expand=True, pady=(0, 20))

        # Text widget com scrollbar para descrições longas
        ...
        scrollbar = tk.Scrollbar(frame_descricao, orient=tk.VERTICAL, command=text_descricao.yview)
        text_descricao.configure(yscrollcommand=scrollbar.set)

        # Inserir texto e tornar somente leitura
        text_descricao.insert("1.0", descricao)
        text_descricao.config(state="disabled")

        # Empacotar
        text_descricao.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        # Função para limpar referência à janela ao fechar
        def fechar_janela_visualizar():
            global janela_visualizar_ativa
            janela_visualizar_ativa = None
        ...
        btn_fechar = tk.Button(
            frame_principal,
            text="Fechar (ESC)",
            command=fechar_janela_visualizar,
            font=("Segoe UI", 10, "bold"),
            bg="#E62F1A",
            fg="white",
            relief="flat",
        ...
        janela_visualizar.bind("<Escape>", lambda e: fechar_janela_visualizar())

        # Focar na janela
    janela_visualizar.deiconify()
    janela_visualizar.focus_set()
    ...
    # Carregamento inicial dos lembretes
    carregar_lembretes()

    # Navegação: incluir o botão Selecionar no Shift+Tab a partir da lista
    def foco_para_btn_selecionar(event):
        btn_selecionar_todos.focus_set()
        return "break"

    lista_lembretes.bind("<Shift-Tab>", foco_para_btn_selecionar)
...
def deletar_lembrete(lista, janela_lembretes):
    ...
    def editar_lembrete(rowid: int, lista: tk.Listbox, parent: tk.Toplevel):
        ...
        cursor.execute("SELECT rowid, data_prometida, descricao FROM promessas WHERE rowid = ?", (rowid,))
        dados = cursor.fetchone()
        if not dados:
            messagebox.showerror("Erro", "Lembrete não encontrado.", parent=parent)
            return

        _, data_atual, desc_atual = dados
        ...
        tk.Button(botoes, text="Cancelar", command=win.destroy).pack(side=tk.LEFT, padx=5)

    # MENU DE CONTEXTO
    menu_contexto = tk.Menu(janela_lembretes, tearoff=0)
    menu_contexto.add_command(label="Visualizar", command=visualizar_lembrete_selecionado)
    menu_contexto.add_command(label="Editar", command=iniciar_edicao)
    menu_contexto.add_separator()
    menu_contexto.add_command(label="Selecionar Todos", command=selecionar_todos)
    ...
    # Botões da interface (com variáveis usadas em debounce e foco)
...
def validar_data(data: str) -> bool:
    ...
    try:
        datetime.strptime(data, "%d/%m/%tk.Y")
        return True
    except ValueError:
        return False
...
def validar_regras_datas(entry_widget_atual):
    ...
    try:
        ...
        hoje = datetime.now().strftime("%d/%m/%tk.Y")

        # Converte strings para objetos datetime para comparação
        if data_recebimento and validar_data(data_recebimento):
            recebimento_obj = datetime.strptime(data_recebimento, "%d/%m/%tk.Y")
        else:
            recebimento_obj = None

        if data_devolucao and validar_data(data_devolucao):
            devolucao_obj = datetime.strptime(data_devolucao, "%d/%m/%tk.Y")
        else:
            devolucao_obj = None

        hoje_obj = datetime.strptime(hoje, "%d/%m/%tk.Y")
...
def validar_campos_obrigatorios(numero_processo, secretaria, data_inicio, data_entrega=None) -> bool:
    ...
    try:
        # Sempre valida e converte a data de início
        data_inicio_dt = datetime.strptime(data_inicio, "%d/%m/%Y")

        # Só valida/convert a data de entrega se houver valor
...
def cadastrar_processo():
    ...
    try:
        ...
        if lembrete_var.get():
            ...
            inseridos_msg = f"Lembrete cadastrado com sucesso em {data_hora_atual}!"
            # Se não houver data explícita no texto, avisar uso da data padrão
            if not re.search(r"\b(\d{1,2})/(\d{1,2})(/(\d{2,4}))?\b", descricao):
                inseridos_msg = f"Lembrete cadastrado em {data_hora_atual} (usando data padrão)"
...
def registrar_promessas(texto, pessoa, data_especifica=None):
    ...
    if pessoa == "Lembrete":
        ...
        try:
            cursor.execute('''
                INSERT INTO promessas (data_prometida, descricao, pessoa)
                VALUES (?, ?, ?)
            ''', (data, texto, pessoa))
            conn.commit()
...
def converter_data_para_banco(data_str, campo_nome):
    ...
    try:
        data_obj = datetime.strptime(data_str, '%d/%m/%tk.Y')
        return data_obj.strftime('%tk.Y-%m-%d')
    except ValueError:
...
def formatar_data_hora_str(data):
    """Formata uma string de data do formato banco para o formato de exibição."""
    if not data:
        return ""
...
def abrir_janela_restaurar():
    ...
    # Ajuste de tamanho conforme imagem solicitada
    janela_restaurar.geometry("860x480")
    # Deixa a janela um pouco mais escura
    try:
        janela_restaurar.configure(bg="#ECEFF1")
    except Exception:
        pass
    ...
    for proc, sec, data_exc in backups:
        ...
        nome_secretaria = secretarias_dict.get(sec, sec)  # Se não encontrar, usa a sigla original
        lista.insert(tk.END, f"{proc} | {nome_secretaria} | Excluído em: {data_exc}")
    ...
    btn_excluir = tk.Button(frame_botoes, text="Excluir", command=excluir_permanentemente, bg="#ff0000",
                         fg="white")
    btn_excluir.pack(side=tk.LEFT, padx=5)
...
def visualizar_registros_excluidos(numeros_processos):
    ...
    # Tamanho base atual (após redução prévia)
    base_width, base_height = 560, 480
    ...
    # Frame de navegação
    frame_nav = tk.Frame(janela_vis)
    # Reduzir espaço do cabeçalho para ganhar altura para Observações
    frame_nav.pack(fill="x", padx=10, pady=2)
    ...
    # Botão para restaurar no cabeçalho (lado esquerdo)
    btn_restaurar_top = tk.Button(frame_nav, text="Restaurar Registro Excluído", command=restaurar_registro_atual)
    btn_restaurar_top.pack(side=tk.LEFT, padx=5)

    lbl_contador = tk.Label(frame_nav, text="", font=("Arial", 10, "bold"))
    ...
    # Botão para reabrir janela de restauração no cabeçalho (à esquerda de Próximo)
    btn_reabrir_top = tk.Button(frame_nav, text="Janela Registro Excluído", command=reabrir_restaurar)
    btn_reabrir_top.pack(side=tk.RIGHT, padx=5)

    # Frame principal com scroll
    main_frame = tk.Frame(janela_vis)
main_frame.pack(fill="both", expand=True, padx=10, pady=4)
    ...
    for label_text, campo_idx in campos_info:
        tk.Label(main_frame, text=label_text, font=("Arial", 10, "bold"), anchor="w").grid(
            row=row, column=0, sticky="w", padx=5, pady=2
        )
        # Reduz wraplength ~20% para acompanhar a janela menor
        lbl_valor = tk.Label(main_frame, text="", font=("Arial", 10), anchor="w", wraplength=320)
        lbl_valor.grid(row=row, column=1, sticky="w", padx=5, pady=2)
        labels_valores[campo_idx] = lbl_valor
        row += 1

    # Descrição (campo maior)
    tk.Label(main_frame, text="Descrição:", font=("Arial", 10, "bold"), anchor="w").grid(
    ...
    descricao_text = tk.Text(main_frame, height=6, width=50, wrap="word", state="normal")
    ...
    def atualizar_campos():
        ...
        if tem_texto:
            ...
            janela_vis.geometry(f"{base_width}x{nova_altura}")
    ...
    def excluir_registro_atual():
        try:
            idx = indice_atual.get()
            numero = registros[idx][1]
            if messagebox.askyesno("Confirmar Exclusão", f"Excluir definitivamente o processo {numero}?\n\nEsta ação não pode ser desfeita!"):
    ...
    # Removido bloco inferior de botões; os botões agora estão no cabeçalho

    # Botão fechar
    btn_fechar = tk.Button(janela_vis, text="Fechar", command=janela_vis.destroy)
    btn_fechar.pack(pady=10)
...
def habilitar_campos_para_edicao():
    ...
    entrada_descricao.config(state='normal')  # Certifique-se de que está 'normal'
...
def manter_campos_pos_atualizacao():
    ...
    entrada_descricao.config(state='disabled')
    entrada_descricao.delete("1.0", tk.END)
...
def buscar_processos():
    ...
    try:
        ...
        if periodo_tempo:
            ...
            padrao_intervalo_com_ano = re.match(r"^(\d{2})(\d{2})(\d{2}|\d{4})?a(\d{2})(\d{2})(\d{2}|\d{4})?$", termo_busca)
        ...
        if not resultados:
            ...
            # Após OK, limpar filtros e recarregar a lista completa
            try:
                limpar_filtros()
            except Exception:
                try:
...
def converter_data_para_exibicao(data_str):
    ...
    try:
        ...
        return data_str  # Retorna o original em caso de erro


def limpar_filtros():
    """Limpa todos os filtros de busca e recarrega a lista completa de processos.

    Reseta os campos de busca, filtros de secretaria, situação e modalidade,
    além de resetar a seleção de itens e a ordenação da tabela.
    """
    try:
        ...
        # Reseta a ordenação para padrão (situação ascendente)
        for col in ordem_colunas_reversa:
            ordem_colunas_reversa[col] = False
        ordenar_coluna('situacao')

        # Garante que a rolagem volte ao topo após limpar
        try:
            tabela.yview_moveto(0)
        except Exception:
            pass
...
def exportar_pdf():
    ...
    try:
        ...
        story.append(Paragraph(f"Gerado em: {datetime.now().strftime('%d/%m/%tk.Y %H:%M')}", estilo_normal))
...
def exportar_excel():
    ...
    try:
        ...
        data_geracao = ws.cell(row=2, column=1, value=f"Gerado em: {datetime.now().strftime('%d/%m/%tk.Y %H:%M')}")
        ...
        for item in tabela.get_children(''):
            ...
            if coluna in ['data_inicio', 'data_entrega', 'data_registro']:
                if valor and valor != "não preenchido":
                    try:
                        data_obj = datetime.strptime(valor, "%d/%m/%tk.Y")
                        valor = data_obj.strftime("%tk.Y%m%d")
                    except ValueError:
                        pass
...
def center_window(window, width=860, height=650):
    ...
    window.maxsize(screen_width, screen_height)  # <-- Isso está permitindo maximizar corretamente


def ativar_botao_atualizar():
    """Ativa o botão Atualizar quando há um processo selecionado para edição"""
    try:
        item_selecionado = tabela.focus()
        if item_selecionado:
            processo = tabela.item(item_selecionado)['values']
...
try:
    from utils.ui_config import get_theme
    theme = get_theme()
    theme.root = janela
    theme.apply_theme("dark")
except Exception as e:
    try:
        print(f"[AVISO] Não foi possível aplicar tema escuro inicial: {e}")
    except:
        pass
...
def ajustar_altura_principal():
    try:
        ...
        # Mantém largura atual (ou padrão) e centraliza dentro da área de trabalho
        width = janela.winfo_width() or 860
...
entrada_numero.grid(row=0, column=1, sticky="ew", padx=10, pady=5)

# tk.Número da Licitação
tk.Label(frame_cadastro, text="Número da Licitação:", bg="#ECEFF1").grid(row=0, column=2, sticky=tk.W)
entrada_licitacao = tk.Entry(frame_cadastro, font=("Segoe UI", 10))
entrada_licitacao.grid(row=0, column=3, sticky=tk.W + tk.E, padx=10, pady=5)
...
entrada_recebimento.grid(row=2, column=1, sticky=tk.W + tk.E, padx=10, pady=5)
...
entrada_secretaria.grid(row=1, column=1, sticky=tk.W + tk.E, padx=10, pady=5)
entrada_secretaria.delete(0, tk.END)
...
# Modalidade
tk.Label(frame_cadastro, text="Modalidade:", bg="#ECEFF1").grid(row=1, column=2, sticky=tk.W)
...
entrada_modalidade.grid(row=1, column=3, sticky=tk.W + tk.E, padx=10, pady=5)
entrada_modalidade.delete(0, tk.END)
...
# Devolução
tk.Label(frame_cadastro, text="Devolução:", bg="#ECEFF1").grid(row=2, column=2, sticky=tk.W)
entrada_devolucao = tk.Entry(frame_cadastro, font=("Segoe UI", 10))
entrada_devolucao.grid(row=2, column=3, sticky=tk.W + tk.E, padx=10, pady=5)
...
entrada_entregue_por.grid(row=3, column=1, sticky=tk.W + tk.E, padx=10, pady=5)
...
# Devolvido a
tk.Label(frame_cadastro, text="Devolvido a:", bg="#ECEFF1").grid(row=3, column=2, sticky=tk.W)
entrada_devolvido_a = AutocompleteEntry(frame_cadastro, nomes_autocomplete, font=("Segoe UI", 10))
entrada_devolvido_a.grid(row=3, column=3, sticky=tk.W + tk.E, padx=10, pady=5)
...
entrada_contratado.grid(row=4, column=3, sticky=tk.W + tk.E, padx=10, pady=5)
...
def configurar_tab_ordem():
    ...
    # ===========================================
    # Em "Nº do contrato", Shift+Tab deve focar o botão "Selecionar Todos" da popup, se existir
    def foco_para_botao_selecionar_todos_popup(event=None):
        try:
            from utils.autocomplete_widget import AutocompleteEntry
        except Exception:
            AutocompleteEntry = globals().get('AutocompleteEntry')
        owner = getattr(AutocompleteEntry, 'current_popup_owner', None)
        if owner and getattr(owner, 'select_all_btn', None):
            ...
            try:
                ...
                pass
        # Fallback para comportamento anterior se não houver popup aberta
        try:
    ...
    entrada_numero.bind("<Shift-Tab>", foco_para_botao_selecionar_todos_popup)
...
frame_cadastro.grid_columnconfigure(0, weight=0)  # Labels não se expandem
frame_cadastro.grid_columnconfigure(1, weight=1)  # Campos se expandem
frame_cadastro.grid_columnconfigure(2, weight=0)  # Labels não se expandem
frame_cadastro.grid_columnconfigure(3, weight=1)  # Campos se expandem
...
frame_obs.grid(row=5, column=1, columnspan=3, sticky=tk.W + tk.E + tk.N + tk.S, padx=2, pady=(10, 0))
...
frame_lembretes = tk.Frame(frame_cadastro, bg="#ECEFF1")
frame_lembretes.grid(row=5, column=0, columnspan=3, pady=(35, 0), sticky=tk.W)  # Posicionado logo abaixo das observações
...
def hover_destacar_botao_lembrete(event=None):
    try:
        botao_lembrete.config(bg="#DDE7F0", relief=tk.RAISED, bd=1)
    except Exception:
        pass

def hover_remover_destaque_botao_lembrete(event=None):
    try:
...
botao_lembrete.bind("<Enter>", hover_destacar_botao_lembrete)
botao_lembrete.bind("<Leave>", hover_remover_destaque_botao_lembrete)

# Destaque visual quando o foco está sobre a palavra "Lembretes"
def destacar_botao_lembrete(event=None):
    try:
        botao_lembrete.config(relief=tk.RAISED, bd=1, fg="#263238", font=("Segoe UI", 9, "bold"))
    except Exception:
        pass

def remover_destaque_botao_lembrete(event=None):
    try:
        botao_lembrete.config(relief=tk.FLAT, bd=0, fg="#37474F", font=("Segoe UI", 9))
    except Exception:
        pass

botao_lembrete.bind("<FocusIn>", destacar_botao_lembrete)
botao_lembrete.bind("<FocusOut>", remover_destaque_botao_lembrete)
...
# Quadradinho do lembrete fica cinza quando tem foco, e volta ao padrão ao sair
def destacar_check_lembrete(event=None):
    try:
        # Borda sólida leve ao focar via teclado; sem sombreado lateral
        check_lembrete.config(relief=tk.SOLID, bd=1, bg="white", activebackground="white")
    except Exception:
        pass

def remover_destaque_check_lembrete(event=None):
    try:
        check_lembrete.config(relief=tk.FLAT, bd=0, bg="white", activebackground="white")
    except Exception:
        pass

# Também aplicar destaque em hover para o checkbox de lembrete
# Remover destaque por hover; manter apenas por foco (TAB/SHIFT+TAB)

check_lembrete.bind("<FocusIn>", destacar_check_lembrete)
check_lembrete.bind("<FocusOut>", remover_destaque_check_lembrete)
...
frame_botoes.pack(anchor="w", fill="x")  # Alinha à esquerda e permite expansão horizontal

# Configurar o grid do frame de botões
for i in range(8):  # 8 colunas para os botões
...
# Botão CADASTRAR (encostado na lateral esquerda)
botao_cadastrar = tk.Button(frame_botoes, text="Cadastrar", command=cadastrar_processo, width=7, **estilo_botao_padrao)
botao_cadastrar.pack(side=tk.LEFT, padx=0, ipadx=3)
add_spacer(frame_botoes, min_width=4)

# Botão LIMPAR
botao_limpar = tk.Button(frame_botoes, text="Limpar", command=limpar_campos, width=5, **estilo_botao_padrao)
botao_limpar.pack(side=tk.LEFT, padx=0, ipadx=3)
add_spacer(frame_botoes, min_width=4)

# Botão EDITAR
botao_editar = tk.Button(frame_botoes, text="Editar", command=editar_processo, width=5, **estilo_botao_padrao)
botao_editar.pack(side=tk.LEFT, padx=0, ipadx=3)
add_spacer(frame_botoes, min_width=4)

# Botão EXCLUIR
botao_excluir = tk.Button(
    frame_botoes, text="Excluir", command=excluir_processo, width=6,
    bg="#e74c3c", fg="white",
    activebackground="#c0392b", activeforeground="white",
    font=("Segoe UI", 10, "bold"), relief=tk.FLAT, bd=0, highlightthickness=2,
    highlightbackground="#e74c3c"
)
...
# Botão EXPORTAR PDF
botao_exportar = tk.Button(frame_botoes, text="Exportar PDF", command=exportar_pdf, width=10, **estilo_botao_padrao)
botao_exportar.pack(side=tk.LEFT, padx=0, ipadx=4)
add_spacer(frame_botoes, min_width=4)

# Botão EXPORTAR TXT
botao_exportar_txt = tk.Button(frame_botoes, text="Exportar TXT", command=exportar_txt, width=10, **estilo_botao_padrao)
...
botao_exportar_excel = tk.Button(frame_botoes, text="Exportar Excel", command=exportar_excel, width=11, **estilo_botao_padrao)
...
botao_banco_dados = tk.Button(frame_botoes, text="Banco de Dados", command=abrir_janela_banco_dados, width=12, **estilo_botao_padrao)
botao_banco_dados.pack(side=tk.LEFT, padx=0, ipadx=4)
add_spacer(frame_botoes, min_width=4)

# Botão RESTAURAR REGISTROS (tamanho exato para caber o texto)
botao_restaurar = tk.Button(
    frame_botoes, text="Restaurar Registros",
    command=abrir_janela_restaurar,
    width=18,  # tamanho ajustado para o texto completo
    **estilo_botao_padrao
)
botao_restaurar.pack(side=tk.LEFT, padx=0, ipadx=6)  # leve padding interno
add_spacer(frame_botoes, min_width=4)
...
frame_busca.pack(anchor="center", pady=2)
...
entrada_busca.grid(row=1, column=0, sticky=tk.W, padx=5, pady=0)
...
entrada_filtro_secretaria.grid(row=1, column=1, sticky=tk.W, padx=1, pady=0)
...
entrada_filtro_situacao.grid(row=1, column=2, sticky=tk.W, padx=5, pady=0)
...
entrada_filtro_modalidade.grid(row=1, column=3, sticky=tk.W, padx=5, pady=0)

botao_buscar = tk.Button(frame_busca, text="Buscar", command=buscar_processos, bg="#394857", fg="#66c0f4",
                      activebackground="#23262e", activeforeground="#66c0f4", width=6,
                      font=("Segoe UI", 10, "bold"), relief=tk.FLAT, bd=0, highlightthickness=1,
                      highlightbackground="#394857")
botao_buscar.grid(row=1, column=4, padx=4)
...
botao_limpar_filtros = tk.Button(frame_busca, text="Limpar", command=limpar_filtros, bg="#394857", fg="#66c0f4",
                              activebackground="#23262e", activeforeground="#66c0f4", width=6,
                              font=("Segoe UI", 10, "bold"), relief=tk.FLAT, bd=0, highlightthickness=1,
                              highlightbackground="#394857")
...
entrada_busca.bind("<Escape>", lambda event: limpar_filtros())
entrada_filtro_secretaria.bind("<Escape>", lambda event: limpar_filtros())
entrada_filtro_situacao.bind("<Escape>", lambda event: limpar_filtros())
...
toggle_btn = tk.Checkbutton(
    ...
    width=2,
    bg="white",
    selectcolor="#37474F",  # Cinza escuro ao marcar
...
def highlight_toggle_on(event=None):
    try:
        # Borda sólida leve ao focar via teclado; sem sombreado lateral
        toggle_btn.config(relief=tk.SOLID, bd=1, bg="white", activebackground="white")
    except Exception:
        pass

def highlight_toggle_off(event=None):
    try:
        toggle_btn.config(relief=tk.FLAT, bd=0, bg="white", activebackground="white")
    except Exception:
        pass

# Apenas foco por teclado
toggle_btn.bind("<FocusIn>", highlight_toggle_on)
toggle_btn.bind("<FocusOut>", highlight_toggle_off)
...
def destacar_toggle_btn(event):
    # Muda o interior do quadradinho para cinza quando em foco
    try:
        toggle_btn.config(bg="#C0C0C0", activebackground="#C0C0C0", selectcolor="#C0C0C0")
    except Exception:
        pass


def remover_destaque_toggle_btn(event):
    # Volta ao interior branco quando não está selecionado nem com foco
    try:
        toggle_btn.config(bg="white", activebackground="#ECEFF1", selectcolor="#95CCC9")
    except Exception:
        pass
...
# Bind do TAB reverso (Shift+Tab): em "Nº do contrato" foca o botão da popup de sugestões
entrada_numero.bind("<Shift-Tab>", foco_para_botao_selecionar_todos_popup)
...
def listar_processos():
    ...
    for row in cursor.fetchall():
        ...
        if valores[0]:
            try:
                valores[0] = datetime.strptime(valores[0], "%Y-%m-%d").strftime("%d/%m/%Y")
            except Exception:
                pass
        if valores[6]:
            try:
                # Garantir que a data de recebimento esteja no formato dia/mês/ano
                if "-" in valores[6]:  # Se estiver no formato ano-mês-dia
                    valores[6] = datetime.strptime(valores[6], "%Y-%m-%d").strftime("%d/%m/%Y")
            except Exception:
                pass
        if valores[7]:
            try:
                # Garantir que a data de devolução esteja no formato dia/mês/ano
                if "-" in valores[7]:  # Se estiver no formato ano-mês-dia
                    valores[7] = datetime.strptime(valores[7], "%Y-%m-%d").strftime("%d/%m/%Y")
            except Exception:
                pass
...
def mostrar_guia_atalhos():
    ...
    # Adicionar binding para a tecla ESC fechar a janela
    janela_atalhos.bind("<Escape>", lambda e: janela_atalhos.destroy())
    ...
    for categoria, lista_atalhos in atalhos.items():
        ...
        for atalho, descricao in lista_atalhos:
            tk.Label(frame_atalhos, text=atalho, width=15, **atalho_style).grid(row=row, column=0, sticky="w", padx=(20, 0))
    ...
    # Estilo do botão padrão da aplicação
    estilo_botao_padrao = {
        "bg": "#394857",
        "fg": "#66c0f4",
        "activebackground": "#23262e",
    ...
    # Botão para fechar com o estilo padrão da aplicação
    tk.Button(frame_principal, text="Fechar", command=janela_atalhos.destroy, 
              width=8, **estilo_botao_padrao).pack(pady=10)
...
def ao_fechar():
    ...
    """
    try:
        # Salva as configurações de largura das colunas
...
try:
    from utils.ui_config import apply_dark_theme_to_all_widgets
    # Aplica tema escuro após construção da UI para cobrir widgets existentes
    janela.after(0, lambda: apply_dark_theme_to_all_widgets(janela))
except Exception:
    pass
